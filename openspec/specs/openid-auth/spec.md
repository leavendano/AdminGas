# Capability: openid-auth

## Purpose
This capability defines the OpenID Connect (OIDC) authentication mechanism for AdminGas. It secures backend endpoints, handles redirection to the Identity Provider, manages the authorization callback flow with PKCE and CSRF protections, and tracks user authentication state within HTTP sessions.
